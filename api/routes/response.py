...
def generate_nrb_report(
    filename: str = Query(None, description="Specific PDF filename to download"),
):
    """
    Return an NRB compliance report PDF generated by the Response Agent.
    If ``filename`` is provided, return that specific report; otherwise return the most recent.
    """
    if not FORENSICS_DIR.exists():
        raise HTTPException(status_code=404, detail="No forensics data available.")

    if filename:
        target = FORENSICS_DIR / filename
        if not target.exists() or not target.name.startswith("NRB_Report_"):
